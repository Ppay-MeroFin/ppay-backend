...
func processAirtimeEvent(ctx context.Context, st *store.Store, ev OutboxEvent) error {
	...
	return completeSuccessfulEvent(ctx, st, ev, payload.CorrelationID, providerTxRef, providerStatus, providerResponsePayload, settledPayload)
}
...
func markFailedAttemptGeneric(ctx context.Context, st *store.Store, ev OutboxEvent, correlationID string, cause error, isDecline bool) error {
    ...
    `, ev.PpayRef, workflowState, "outbox-worker", reasonCode, correlationID, failurePayload, time.Now().UTC())
	if err != nil {
		return err
	}
